...
远端配置页面把已保存对象目录与当前编辑对象分开。目录负责选择、设为默认和删除，编辑器负责字段与唯一的主要提交动作。

- 从目录选择对象时，用服务端返回值完整填充编辑器；API 密钥等只写字段只展示掩码，不回填明文。
- 已保存对象的身份字段不可直接编辑。新身份通过新建入口创建，避免服务端的 upsert 语义被误解为重命名。
- 提交按钮只在必填字段有效且内容发生变化时启用。
- 每类请求维护独立的 pending 状态；请求期间锁定会冲突的控件，并在发起按钮上显示 `loading`。
- 删除、清空和覆盖在服务端确认后再同步本地状态；失败时保留用户正在编辑的内容。
- 客户端运行时必须识别承载确认结果的协议帧。TypeScript 声明不能替代运行时校验与消息分发。
- 状态机至少覆盖初始、脏数据、提交中、成功确认和失败五种状态，并为载荷构造与确认帧写自动化测试。
...
- 密集列表中的行内按钮 hover 不使用位移、独立背景或描边；仅通过文字色或图标色反馈，避免把一行切成多个视觉按钮。
