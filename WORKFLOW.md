...
2. **不存在部署目录**：`~/apps/loreweaver-web`、`~/apps/loreweaver` 已于 2026-08-24 删除（残留 root 文件在 `~/apps/.loreweaver-*-trash`）。代码、部署全在本 repo；**数据在独立目录 `~/loreweaver-data`**（compose 里 `${LORE_DATA_DIR:-/home/melon/loreweaver-data}:/data`），不进 repo。
...
2026-08-24 根治：废除"双 checkout"模式——数据迁到独立目录 `~/loreweaver-data`，compose 的引擎上下文指向 `~/repos/loreweaver` 源码 repo，部署目录整体删除。现在线上跑的就是 repo 的代码，不存在第二份可漂移的副本。
